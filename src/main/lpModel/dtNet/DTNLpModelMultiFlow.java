...
public class DTNLpModelMultiFlow{
	...
	public final int[] flowByNode = {3,0,2};

	private DTNLpModelMultiFlow(
	...
	public static DTNLpModelMultiFlow generateFromFile(String path, int requiredPaths, int requiredCapacity) throws Exception{
		...
		DTNLpModelMultiFlow model = new DTNLpModelMultiFlow(
	 ...
	 * 
	 * */
	protected Pair<Integer,ListenableDirectedWeightedGraph<Vertex,Edge>> solve() throws IOException, LpSolveException{
...
//	}
	...
	public String generateLPFormatString(){
		List<List<GraphPath<Vertex, Edge>>> paths = new ArrayList<List<GraphPath<Vertex,Edge>>>(this.flowByNode.length); 
		
		for(int i=0; i< this.flowByNode.length; i++)
			paths.add(getAllSinglePath(graph, vertexs[i],vertexs[vertexs.length - 1 ]));
...
//		st.append(" +" + paths.get(i).getWeight()+" P"+i );
	...
	//amount of path constraint
	//aca hay un problema cuándo no hay caminos de un nodo a otro
	for(int j=0; j<paths.size(); j++){
		//else isn't required paths from node j
		if(flowByNode[j] > 0){
		st.append("r_" + constraintNumber + ":"); constraintNumber++;
		for(int i=0; i<paths.get(j).size();i++)
			st.append(" +P"+j+i);
		st.append(" >= " + AMOUNT_OF_PATH + ";\n");
		}
	}
	...
	for(int j=0; j<paths.size();j++){
		if(flowByNode[j] > 0 ){
			for(int i=0;i<paths.get(j).size();i++){
				st.append("r_"+ constraintNumber + ":"); constraintNumber++;
				st.append("+" + paths.get(j).get(i).getEdgeList().size() + " P"+ j + i + " <=");
				for(int k=0; k<paths.get(j).get(i).getEdgeList().size(); k++)
					st.append(" +" + paths.get(j).get(i).getEdgeList().get(k).name);
				st.append(";\n");
			}
		}
	}
		...
		for(int j=0;j<paths.size();j++){
			if(flowByNode[j] > 0){
				for(int k=0; k<paths.get(j).size();k++)
					if(paths.get(j).get(k).getEdgeList().contains(edges[i]))
						aux.append(" +P"+ j + k);
			}
		}
		if(aux.length() == 0)
...
//		}
	...
	//source node flow constraint
//	st.append("r_"+ constraintNumber + ": k0 =" + getOutflow(vertexs[0]) + ";\n"); constraintNumber++;
	
	//flow continuity constraint
	for(int i=0; i<vertexs.length-1; i++){		
		st.append("r_"+ constraintNumber + ":" + ((flowByNode.length > i && flowByNode[i]>0)? "+k" + i :"") + getInflow(vertexs[i]) + " =" + getOutflow(vertexs[i]) + ";\n"); constraintNumber++;
	}

//	//target node flow constraint -> It isn't necessary if put flow origin constraints.
//	st.append("r_"+ constraintNumber+ ":");
//	for(int i=0; i<flowByNode.length;i++)
//		st.append(" +k" + i); 	

//	st.append(" = " + getInflow(vertexs[vertexs.length - 1]) + ";\n"); constraintNumber++;
	
	//The network has capacity al leat km+..+Kn for routing flow generated at nodes n...m
	for(int i=0; i<flowByNode.length;i++){
		if(flowByNode[i]>0){
			st.append("r_"+ constraintNumber+ ": +k" + i + " >= " + flowByNode[i] + ";\n"); 
			constraintNumber++;
		}
	}
