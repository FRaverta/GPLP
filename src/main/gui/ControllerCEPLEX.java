...
import main.lpModel.wireNet.WNLpModelCEPLEX;
...
public class ControllerCEPLEX extends ControllerFather{
	...
	public ControllerCEPLEX(){
		try {
			...
			WNLpModelCEPLEX model = WNLpModelCEPLEX.DefaultLPModel();
	...
	public void loadExample1(boolean strongSharedEge, double sharedEdegeWeighing){	
		try {
			WNLpModelCEPLEX model = WNLpModelCEPLEX.LpModelFromGraph(
			...
			e.printStackTrace();
			Parameters.report.writelnRed(e.toString());
		}
	...
	class GenerateLpModelTask implements Runnable{
		...
		public void run() {
			try {
				...
				WNLpModelCEPLEX model = WNLpModelCEPLEX.generateLpModel(amount_of_vertex, edge_density,amount_of_path, max_weight_edge);
				if(model == null){
					Parameters.report.writeln(":( :( :( :( :( :(:( :(:( :(:( :(:( :(:( :( ");
	...
	public static void main(String args[]){
		ControllerCEPLEX c = new ControllerCEPLEX();
	}
